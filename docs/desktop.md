...
**Crate:** `openparlant-desktop`
**Identifier:** `ai.openparlant.desktop`
...
| **Open Config Directory** | Opens `~/.openparlant/` in the OS file manager |
...
Opens a native file picker for `.toml` files. Validates the selected file as an `AgentManifest`, copies it to `~/.openparlant/agents/{name}/agent.toml`, and spawns the agent. Returns the agent name on success.

### `import_skill_file`

Opens a native file picker for skill files (`.md`, `.toml`, `.py`, `.js`, `.wasm`). Copies the file to `~/.openparlant/skills/` and triggers a hot-reload of the skill registry.
...
Opens `~/.openparlant/` or `~/.openparlant/logs/` in the OS file manager.
...
| Variable | Effect |
|----------|--------|
| `RUST_LOG` | Controls tracing verbosity. Defaults to `openparlant=info,tauri=info` if unset. |
