...
List OFP (SiliCrew Protocol) wire peers and their connection status.
...
SiliCrew maintains a built-in catalog of 51+ models across 20 providers. These endpoints allow you to browse available models, check provider authentication status, and resolve model aliases.
...
Install a skill from ClawHub. Downloads, verifies SHA256 checksum, scans for prompt injection, and converts SKILL.md format to SiliCrew skill.toml automatically.
...
SiliCrew supports both Model Context Protocol (MCP) for tool interoperability and Agent-to-Agent (A2A) protocol for cross-system agent communication.
...
MCP HTTP transport endpoint. Accepts JSON-RPC 2.0 requests and exposes SiliCrew tools via the MCP protocol to external clients.
...
{
  "name": "SiliCrew",
  "description": "SiliCrew Agent Operating System",
...
SiliCrew maintains a Merkle hash chain audit trail for all security-relevant operations. These endpoints allow inspection and verification of the audit log integrity.
...
SiliCrew exposes an OpenAI-compatible API for drop-in integration with tools that support the OpenAI API format (Cursor, Continue, Open WebUI, etc.).
...
**Model resolution** (the `model` field maps to an SiliCrew agent):
