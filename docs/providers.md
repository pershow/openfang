...
SiliCrew auto-detects which providers have API keys configured at boot. Any model whose provider is authenticated becomes immediately available. Local providers (Ollama, vLLM, LM Studio) require no key at all.
...
**Notes:** SiliCrew auto-discovers models from a running Ollama instance and merges them into the catalog with `Local` tier and zero cost. Any model you pull becomes usable immediately.
...
SiliCrew can automatically select the cheapest model capable of handling each query. This is configured per-agent via `ModelRoutingConfig`.
...
SiliCrew tracks the cost of every LLM call and can enforce per-agent spending quotas.
