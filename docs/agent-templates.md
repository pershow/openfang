...
```bash
silicrew spawn orchestrator
silicrew spawn coder
silicrew spawn --template agents/writer/agent.toml
```
...
```bash
silicrew spawn security-auditor
...
```bash
silicrew spawn legal-assistant
...
```bash
silicrew spawn email-assistant
...
```bash
silicrew spawn customer-support
...
```bash
silicrew spawn sales-assistant
...
```bash
silicrew spawn meeting-assistant
...
```bash
silicrew spawn personal-finance
...
```bash
silicrew spawn home-automation
...
silicrew spawn coder --name "backend-coder"

# Spawn from a TOML file path
silicrew spawn --template agents/custom/my-agent.toml

# List running agents
silicrew agents

# Send a message
silicrew message <agent-id> "Write a function to parse TOML files"

# Kill an agent
silicrew kill <agent-id>
```
