...
```bash
GROQ_API_KEY=<key> target/release/silicrew.exe start &
...
## Architecture Notes
- **Don't touch `silicrew-cli`** — user is actively building the interactive CLI
...
## Common Gotchas
- `silicrew.exe` may be locked if daemon is running — use `--lib` flag or kill daemon first
