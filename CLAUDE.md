...
- Config: `~/.openparlant/config.toml`
- Default API: `http://127.0.0.1:4200`
- CLI binary: `target/release/openparlant.exe` (or `target/debug/openparlant.exe`)
...
```bash
GROQ_API_KEY=<key> target/release/openparlant.exe start &
...
## Architecture Notes
- **Don't touch `openparlant-cli`** — user is actively building the interactive CLI
...
## Common Gotchas
- `openparlant.exe` may be locked if daemon is running — use `--lib` flag or kill daemon first
