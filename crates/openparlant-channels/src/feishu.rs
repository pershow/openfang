...
pub struct FeishuAdapter {
    ...
    /// Pending "Typing" message reaction IDs (message_id → reaction_id) for `clear_typing`.
    typing_reactions: Arc<RwLock<HashMap<String, String>>>,
}
...
impl FeishuAdapter {
    ...
    pub fn new(app_id: String, app_secret: String, webhook_port: u16) -> Self {
        ...
        Self {
            ...
            typing_reactions: Arc::new(RwLock::new(HashMap::new())),
        }
    }
    ...
    /// Feishu [`emoji_type`](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/message-reaction/emojis-introduce) for the "Typing" sticker reaction.
    const TYPING_REACTION_EMOJI: &'static str = "Typing";

    /// POST `/open-apis/im/v1/messages/:message_id/reactions` — add a message reaction.
    async fn api_add_message_reaction(
        &self,
        message_id: &str,
        emoji_type: &str,
        ...
        let token = self.get_token().await?;
        let url = self.api_url(&format!("/open-apis/im/v1/messages/{message_id}/reactions"));
        let body = serde_json::json!({
        ...
        let resp_body: serde_json::Value = if resp.status().is_success() {
            resp.json().await?
        } else {
            let status = resp.status();
            let text = resp.text().await.unwrap_or_default();
            warn!(
                "{} add message reaction HTTP {status}: {text}",
                self.region.label()
            );
            return Ok(None);
        };
        let code = resp_body["code"].as_i64().unwrap_or(-1);
        if code == 0 {
            let rid = resp_body["data"]["reaction_id"]
                .as_str()
                .map(std::string::ToString::to_string);
            return Ok(rid);
        }

        // Duplicate reaction in flight — treat as already typing
        if code == 231015 {
            debug!(
                "{} add reaction duplicate in flight for message {message_id}",
                self.region.label()
            );
            return Ok(None);
        }
        ...
        let msg = resp_body["msg"].as_str().unwrap_or("unknown error");
        warn!(
            "{} add message reaction failed (code {code}): {msg}",
            self.region.label()
        );
        Ok(None)
    }
    ...
    async fn api_delete_message_reaction(
        &self,
        message_id: &str,
        reaction_id: &str,
    ) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if !resp.status().is_success() {
            ...
            let resp_body = resp.text().await.unwrap_or_default();
            return Err(format!(
                "{} delete message reaction error {status}: {resp_body}",
                self.region.label()
            )
            .into());
        }
        ...
        if code != 0 {
            let msg = resp_body["msg"].as_str().unwrap_or("unknown error");
            warn!(
                "{} delete message reaction API error: {msg} (code {code})",
                self.region.label()
            );
        }
        Ok(())
...
impl ChannelAdapter for FeishuAdapter {
    ...
    async fn send_typing(
        ...
        let Some(mid) = reply_to_message_id.filter(|s| !s.is_empty()) else {
            return Ok(());
        };
        {
            let guard = self.typing_reactions.read().await;
            if guard.contains_key(mid) {
                return Ok(());
            }
        }
        let opt_rid = match self
            .api_add_message_reaction(mid, Self::TYPING_REACTION_EMOJI)
            .await
        {
            Ok(v) => v,
            Err(e) => return Err(format!("{e}").into()),
        };
        if let Some(rid) = opt_rid {
            let mut guard = self.typing_reactions.write().await;
            guard.insert(mid.to_string(), rid);
        }
        Ok(())
    }
    ...
    async fn clear_typing(
        ...
        if let Some(rid) = rid {
            let _ = self.api_delete_message_reaction(mid, &rid).await;
        }
        Ok(())
    }
