...
//!
//! Launched by `silicrew init` (without `--quick`). Takes over the terminal,
...
use crate::tui::theme;
use silicrew_runtime::model_catalog::ModelCatalog;
use silicrew_types::model_catalog::ModelTier;
...
struct State {
    ...
    openclaw_scan: Option<silicrew_migrate::openclaw::ScanResult>,
    migration_report: Option<silicrew_migrate::report::MigrationReport>,
...
impl State {
    ...
    fn is_provider_detected(&self, prov_idx: usize) -> bool {
        ...
        if p.name == "claude-code" {
            return silicrew_runtime::drivers::claude_code::claude_code_available();
        }
        (!p.env_var.is_empty() && std::env::var(p.env_var).is_ok())
...
pub fn run() -> InitResult {
    ...
    let (migrate_tx, migrate_rx) =
        std::sync::mpsc::channel::<Result<silicrew_migrate::report::MigrationReport, String>>();
...
fn handle_migration_key(
    ...
    migrate_tx: &std::sync::mpsc::Sender<
        Result<silicrew_migrate::report::MigrationReport, String>,
    >,
) {
                ...
                if yes {
                    ...
                    std::thread::spawn(move || {
                        let options = silicrew_migrate::MigrateOptions {
                            source: silicrew_migrate::MigrateSource::OpenClaw,
                            source_dir,
                            target_dir,
                            dry_run: false,
                        };
                        let result = silicrew_migrate::run_migration(&options)
                            .map_err(|e| format!("{e}"));
...
{routing_section}"#,
    ...
    match crate::start_daemon_background(None) {
        Ok(url) => {
...
/// Check if the `silicrew-desktop` binary exists next to the current exe.
...
fn draw_migration_done(f: &mut Frame, area: Rect, state: &State) {
    ...
    if let Some(ref error) = state.migration_error {
        ...
        use silicrew_migrate::report::ItemKind;
...
fn draw_routing_pick(f: &mut Frame, area: Rect, state: &mut State, tier: usize) {
        ...
        .enumerate()
    {
        if t == tier {
            ...
            let display = silicrew_types::truncate_str(short, 14);
            summary_spans.push(Span::styled(
                format!("{name}:{display}"),
                Style::default().fg(*c),
            ));
