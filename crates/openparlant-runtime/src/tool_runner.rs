...
use silicrew_types::taint::{TaintLabel, TaintSink, TaintedValue};
use silicrew_types::tool::{ToolDefinition, ToolResult};
use silicrew_types::tool_compat::normalize_tool_name;
...
pub async fn execute_tool(
    ...
    exec_policy: Option<&silicrew_types::config::ExecPolicy>,
    ...
    if let Some(kh) = kernel {
        ...
        if needs_approval {
            ...
            let summary = format!(
                "{}: {}",
                tool_name,
                silicrew_types::truncate_str(&input_str, 200)
            );
    ...
    let result = match tool_name {
        ...
        "shell_exec" => {
            ...
            let is_full_exec = exec_policy
                .is_some_and(|p| p.mode == silicrew_types::config::ExecSecurityMode::Full);
            if !is_full_exec {
        ...
        other => {
            ...
            // Fallback 2: Skill registry tool providers
                ...
                if let Some(skill) = registry.find_tool_provider(other) {
                    ...
                    match silicrew_skills::loader::execute_skill_tool(
                        &skill.manifest,
                        &skill.path,
                        other,
                        input,
...
async fn tool_shell_exec(
    ...
    let use_direct_exec = exec_policy
        .map(|p| p.mode == silicrew_types::config::ExecSecurityMode::Allowlist)
...
fn parse_entity_type(s: &str) -> silicrew_types::memory::EntityType {
    use silicrew_types::memory::EntityType;
...
fn parse_relation_type(s: &str) -> silicrew_types::memory::RelationType {
    use silicrew_types::memory::RelationType;
    match s.to_lowercase().as_str() {
...
async fn tool_knowledge_add_entity(
    ...
    let entity = silicrew_types::memory::Entity {
...
async fn tool_knowledge_add_relation(
    ...
    let relation = silicrew_types::memory::Relation {
...
async fn tool_knowledge_query(
    ...
    let pattern = silicrew_types::memory::GraphPattern {
        source,
        relation,
        target,
        max_depth,
...
async fn tool_media_describe(
    ...
    let attachment = silicrew_types::media::MediaAttachment {
        media_type: silicrew_types::media::MediaType::Image,
        mime_type: mime.to_string(),
        source: silicrew_types::media::MediaSource::Base64 {
...
async fn tool_image_generate(
    ...
    let model = match model_str {
        "dall-e-3" | "dalle3" | "dalle-3" => silicrew_types::media::ImageGenModel::DallE3,
        "dall-e-2" | "dalle2" | "dalle-2" => silicrew_types::media::ImageGenModel::DallE2,
        "gpt-image-1" | "gpt_image_1" => silicrew_types::media::ImageGenModel::GptImage1,
        _ => {
            return Err(format!(
    ...
    let request = silicrew_types::media::ImageGenRequest {
...
async fn tool_speech_to_text(
    ...
    use silicrew_types::media::{MediaAttachment, MediaSource, MediaType};
...
mod tests {
    ...
    async fn test_file_read_missing() {
        let bad_path = std::env::temp_dir()
            .join("silicrew_test_nonexistent_99999")
            .join("file.txt");
    ...
    async fn test_capability_enforcement_allowed() {
        ...
        let bad_path = std::env::temp_dir()
            .join("silicrew_test_nonexistent_12345")
            .join("file.txt");
        let result = execute_tool(
            "test-id",
            "file_read",
    ...
    async fn test_canvas_present_tool() {
        ...
        let tmp = std::env::temp_dir().join("silicrew_canvas_test");
