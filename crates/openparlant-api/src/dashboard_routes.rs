...
pub(crate) fn current_dashboard_user(state: &AppState, headers: &HeaderMap) -> Result<DashboardUser, ApiJson> {
...
pub(crate) fn internal_error(error: OpenFangError) -> ApiJson {
...
fn load_tenant_llm_models(
    ...
    match state.control_store.get_setting(&tenant_llm_models_key(tenant_id)) {
        Ok(Some(setting)) => Ok(
            serde_json::from_str::<Vec<serde_json::Value>>(&setting.value_json)
                .unwrap_or_default(),
        ),
        Ok(None) => Ok(Vec::new()),
...
fn save_tenant_llm_models(
    ...
    state
        .control_store
        .upsert_setting(&tenant_llm_models_key(tenant_id), &serde_json::to_string(models).unwrap_or_else(|_| "[]".to_string()))
        .map_err(internal_error)?;
    Ok(())
}
...
fn extract_mentions(content: &str) -> Vec<String> {
    content
        ...
        .map(|value| {
            value
                .trim_matches(|ch: char| !ch.is_alphanumeric() && ch != '_' && ch != '-' && !('\u{4e00}'..='\u{9fff}').contains(&ch))
                .to_string()
        })
...
pub async fn self_create_company(
    ...
    let tenant = DashboardTenant {
        ...
        slug: slugify_company_name(&req.name),
        is_active: true,
        created_at: chrono::Utc::now(),
...
pub async fn update_tenant(
    ...
    tenant.name = req.name.trim().to_string();
    if let Err(error) = state.control_store.upsert_tenant(&tenant) {
        return internal_error(error);
    }
...
pub async fn delete_tenant(
    ...
    match state.control_store.delete_tenant(&tenant_id) {
        ...
        Ok(None) => api_error(StatusCode::NOT_FOUND, "Tenant not found"),
        Err(OpenFangError::InvalidInput(message)) => api_error(StatusCode::BAD_REQUEST, message),
        Err(error) => internal_error(error),
    }
}
...
pub async fn get_system_setting(
    ...
    match state.control_store.get_setting(&key) {
        Ok(Some(setting)) => (
            ...
            Json(serde_json::json!({
                "key": setting.key,
                "value": serde_json::from_str::<serde_json::Value>(&setting.value_json).unwrap_or_else(|_| serde_json::json!({})),
...
pub async fn upsert_system_setting(
    ...
    match state
        .control_store
        .upsert_setting(&key, &req.value.to_string())
    {
        Ok(setting) => (
            StatusCode::OK,
            Json(serde_json::json!({
...
pub async fn enterprise_org_departments(
    ...
    let _tenant_filter = if user.role == "platform_admin" {
        query.tenant_id.as_deref()
    } else {
        user.tenant_id.as_deref()
    };
...
pub async fn enterprise_org_members(
    ...
    let users = match state.control_store.list_users(tenant_filter) {
        Ok(users) => users,
        Err(error) => return internal_error(error),
    };
    let items = users
...
pub async fn enterprise_org_sync(
    ...
    (
        ...
        Json(serde_json::json!({
            "departments": 0,
            "members": 0,
            "message": "Org sync is not configured in this build yet",
        })),
    )
}
...
pub async fn enterprise_tenant_quotas(
    ...
    (
        ...
        Json(serde_json::json!({
            ...
            "default_max_llm_calls_per_day": extra.get("default_max_llm_calls_per_day").and_then(|v| v.as_i64()).unwrap_or(100),
            "min_heartbeat_interval_minutes": extra.get("min_heartbeat_interval_minutes").and_then(|v| v.as_i64()).unwrap_or(120),
            "default_max_triggers": extra.get("default_max_triggers").and_then(|v| v.as_i64()).unwrap_or(20),
            "min_poll_interval_floor": extra.get("min_poll_interval_floor").and_then(|v| v.as_i64()).unwrap_or(5),
            "max_webhook_rate_ceiling": extra.get("max_webhook_rate_ceiling").and_then(|v| v.as_i64()).unwrap_or(5),
        })),
    )
}
...
pub async fn update_enterprise_tenant_quotas(
    ...
    tenant.default_agent_ttl_hours = req
        .get("default_agent_ttl_hours")
        .and_then(|v| v.as_i64())
...
pub async fn enterprise_stats(
    ...
    let pending_approvals = state
        ...
        .filter(|approval| {
            tenant_agents
                .iter()
                .any(|entry| entry.id.to_string() == approval.agent_id || entry.name == approval.agent_id)
        })
        .count();
    (
        StatusCode::OK,
...
pub async fn enterprise_llm_providers(
    ...
    let catalog = match state.kernel.model_catalog.read() {
        Ok(catalog) => catalog,
        Err(_) => return api_error(StatusCode::INTERNAL_SERVER_ERROR, "Model catalog unavailable"),
    };
    let items = catalog
        .list_providers()
        .iter()
...
pub async fn enterprise_update_llm_model(
    ...
    if let Some(object) = models[index].as_object_mut() {
        for key in ["provider", "model", "label", "base_url", "supports_vision", "max_output_tokens", "enabled"] {
...
pub async fn plaza_list_posts(
        ...
        .list_plaza_posts(query.tenant_id.as_deref(), limit, offset)
    {
        Ok(posts) => (
            StatusCode::OK,
            Json(serde_json::json!(posts.into_iter().map(|post| serde_json::json!({
                "id": post.id,
...
fn notify_mentions_for_content(
    ...
    for agent in state.kernel.registry.list() {
        ...
        if mentions.iter().any(|mention| mention.eq_ignore_ascii_case(&agent_name))
                                    ...
                                    .and_then(|value| value.as_str())
                                {
                                    if creator_id != user.user_id {
                                        send_social_notification(
                                            ...
                                            creator_id,
                                            "plaza_comment",
                                            format!("{} commented on {}'s post", comment.author_name, post.author_name),
                                            Some(comment.content.chars().take(160).collect()),
                                            Some(link.clone()),
                                            Some(user.user_id.clone()),
                                            Some(comment.author_name.clone()),
                    ...
                    if post.author_id != user.user_id {
                        let target_user_id = if post.author_type == "human" {
                            ...
                            state
                                ...
                                .and_then(|agent| {
                                    agent.manifest.metadata.get("creator_user_id").and_then(|value| value.as_str()).map(|value| value.to_string())
                                })
                        };
