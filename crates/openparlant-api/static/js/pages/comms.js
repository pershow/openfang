...
function commsPage() {
  return {
    ...
    async loadData() {
      ...
      try {
        var results = await Promise.all([
          SiliCrewAPI.get('/api/comms/topology'),
          SiliCrewAPI.get('/api/comms/events?limit=200')
        ]);
    ...
    startSSE() {
      ...
      var self = this;
      var url = SiliCrewAPI.baseUrl + '/api/comms/events/stream';
      if (SiliCrewAPI.apiKey) url += '?token=' + encodeURIComponent(SiliCrewAPI.apiKey);
    ...
    async refreshTopology() {
      try {
        this.topology = await SiliCrewAPI.get('/api/comms/topology');
      } catch(e) { /* silent */ }
    },
    ...
    async submitSend() {
      ...
      try {
        await SiliCrewAPI.post('/api/comms/send', {
          from_agent_id: this.sendFrom,
          to_agent_id: this.sendTo,
          message: this.sendMsg
        });
        SiliCrewToast.success('Message sent');
        this.showSendModal = false;
      } catch(e) {
        SiliCrewToast.error(e.message || 'Send failed');
      }
    ...
    async submitTask() {
      ...
      try {
        ...
        await SiliCrewAPI.post('/api/comms/task', body);
        SiliCrewToast.success('Task posted');
        this.showTaskModal = false;
      } catch(e) {
        SiliCrewToast.error(e.message || 'Task failed');
      }
