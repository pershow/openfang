  ...
  return {
      ...
      this.$watch('inputText', function(val) {
        ...
        if (modelMatch) {
          ...
          if (!self.modelPickerList.length) {
            SiliCrewAPI.get('/api/models').then(function(data) {
    ...
    toggleModelSwitcher() {
      ...
      SiliCrewAPI.get('/api/models').then(function(data) {
        ...
        SiliCrewToast.error('Failed to load models: ' + e.message);
      });
    },
    ...
    switchModel(model) {
      ...
      SiliCrewAPI.put('/api/agents/' + this.currentAgent.id + '/model', { model: model.id }).then(function(resp) {
        ...
        SiliCrewToast.success('Switched to ' + (model.display_name || model.id));
        self.showModelSwitcher = false;
        self.modelSwitching = false;
      }).catch(function(e) {
        SiliCrewToast.error('Switch failed: ' + e.message);
        self.modelSwitching = false;
      });
    },
    ...
    fetchCommands: function() {
      var self = this;
      SiliCrewAPI.get('/api/commands').then(function(data) {
      ...
      switch (cmd) {
        ...
        case '/new':
          if (self.currentAgent) {
            SiliCrewAPI.post('/api/agents/' + self.currentAgent.id + '/session/reset', {}).then(function() {
              self.messages = [];
              SiliCrewToast.success('Session reset');
            }).catch(function(e) { SiliCrewToast.error('Reset failed: ' + e.message); });
          }
          break;
        case '/compact':
          if (self.currentAgent) {
            self.messages.push({ id: ++msgId, role: 'system', text: 'Compacting session...', meta: '', tools: [] });
            SiliCrewAPI.post('/api/agents/' + self.currentAgent.id + '/session/compact', {}).then(function(res) {
              self.messages.push({ id: ++msgId, role: 'system', text: res.message || 'Compaction complete', meta: '', tools: [] });
              self.scrollToBottom();
            }).catch(function(e) { SiliCrewToast.error('Compaction failed: ' + e.message); });
          }
          break;
        case '/stop':
          if (self.currentAgent) {
            SiliCrewAPI.post('/api/agents/' + self.currentAgent.id + '/stop', {}).then(function(res) {
              self.messages.push({ id: ++msgId, role: 'system', text: res.message || 'Run cancelled', meta: '', tools: [] });
              self.sending = false;
              self.scrollToBottom();
            }).catch(function(e) { SiliCrewToast.error('Stop failed: ' + e.message); });
          }
          break;
        case '/usage':
          if (self.currentAgent) {
        ...
        case '/context':
          // Send via WS command
          if (self.currentAgent && SiliCrewAPI.isWsConnected()) {
            SiliCrewAPI.wsSend({ type: 'command', command: 'context', args: '' });
          } else {
        ...
        case '/verbose':
          if (self.currentAgent && SiliCrewAPI.isWsConnected()) {
            SiliCrewAPI.wsSend({ type: 'command', command: 'verbose', args: cmdArgs });
          } else {
            self.messages.push({ id: ++msgId, role: 'system', text: 'Not connected. Connect to an agent first.', meta: '', tools: [] });
            self.scrollToBottom();
          }
        ...
        case '/queue':
          if (self.currentAgent && SiliCrewAPI.isWsConnected()) {
            SiliCrewAPI.wsSend({ type: 'command', command: 'queue', args: '' });
          } else {
          ...
          break;
        case '/status':
          SiliCrewAPI.get('/api/status').then(function(s) {
        ...
        case '/model':
          if (self.currentAgent) {
            if (cmdArgs) {
              SiliCrewAPI.put('/api/agents/' + self.currentAgent.id + '/model', { model: cmdArgs }).then(function(resp) {
                ...
                self.scrollToBottom();
              }).catch(function(e) { SiliCrewToast.error('Model switch failed: ' + e.message); });
            } else {
        ...
        case '/exit':
          ...
          break;
        case '/budget':
          SiliCrewAPI.get('/api/budget').then(function(b) {
          ...
          break;
        case '/peers':
          SiliCrewAPI.get('/api/network/status').then(function(ns) {
          ...
          break;
        case '/a2a':
          SiliCrewAPI.get('/api/a2a/agents').then(function(res) {
    ...
    async loadSession(agentId) {
      var self = this;
      try {
        var data = await SiliCrewAPI.get('/api/agents/' + agentId + '/session');
    ...
    async loadSessions(agentId) {
      try {
        var data = await SiliCrewAPI.get('/api/agents/' + agentId + '/sessions');
    ...
    async createSession() {
      ...
      try {
        await SiliCrewAPI.post('/api/agents/' + this.currentAgent.id + '/sessions', {
        ...
        this.messages = [];
        this.scrollToBottom();
        if (typeof SiliCrewToast !== 'undefined') SiliCrewToast.success('New session created');
      } catch(e) {
        if (typeof SiliCrewToast !== 'undefined') SiliCrewToast.error('Failed to create session');
      }
    },
    ...
    async switchSession(sessionId) {
      if (!this.currentAgent) return;
      try {
        await SiliCrewAPI.post('/api/agents/' + this.currentAgent.id + '/sessions/' + sessionId + '/switch', {});
        this.messages = [];
        await this.loadSession(this.currentAgent.id);
        await this.loadSessions(this.currentAgent.id);
        ...
        if (typeof SiliCrewToast !== 'undefined') SiliCrewToast.error('Failed to switch session');
      }
    },
    ...
    async sendMessage() {
      ...
      if (this.attachments.length) {
        for (var i = 0; i < this.attachments.length; i++) {
          ...
          att.uploading = true;
          try {
            var uploadRes = await SiliCrewAPI.upload(this.currentAgent.id, att.file);
            ...
            SiliCrewToast.error('Failed to upload ' + att.file.name);
    ...
    async _sendPayload(finalText, uploadedFiles, msgImages) {
      ...
      if (SiliCrewAPI.wsSend(wsPayload)) {
      ...
      // HTTP fallback
      if (!SiliCrewAPI.isWsConnected()) {
        SiliCrewToast.info('Using HTTP mode (no streaming)');
      }
      ...
      try {
        ...
        var res = await SiliCrewAPI.post('/api/agents/' + this.currentAgent.id + '/message', httpBody);
    ...
    stopAgent: function() {
      if (!this.currentAgent) return;
      var self = this;
      SiliCrewAPI.post('/api/agents/' + this.currentAgent.id + '/stop', {}).then(function(res) {
    ...
    killAgent() {
      ...
      SiliCrewToast.confirm('Stop Agent', 'Stop agent "' + name + '"? The agent will be shut down.', async function() {
        try {
          await SiliCrewAPI.del('/api/agents/' + self.currentAgent.id);
          SiliCrewAPI.wsDisconnect();
          self._wsAgent = null;
          self.currentAgent = null;
          self.messages = [];
          SiliCrewToast.success('Agent "' + name + '" stopped');
    ...
    addFiles(files) {
      ...
      for (var i = 0; i < files.length; i++) {
        ...
        if (file.size > 10 * 1024 * 1024) {
          SiliCrewToast.warn('File "' + file.name + '" exceeds 10MB limit');
          continue;
        }
        ...
        if (!typeOk) {
          SiliCrewToast.warn('File type not supported: ' + file.name);
          continue;
        }
        var preview = null;
    ...
    startRecording: async function() {
      ...
      try {
        ...
        if (typeof SiliCrewToast !== 'undefined') SiliCrewToast.error('Microphone access denied');
      }
    },
    ...
    _handleRecordingComplete: async function() {
      ...
      try {
        ...
        var upload = await SiliCrewAPI.upload(this.currentAgent.id, file);
        ...
        this.messages = this.messages.filter(function(m) { return !m.thinking || m.role !== 'system'; });
        if (typeof SiliCrewToast !== 'undefined') SiliCrewToast.error('Failed to upload audio: ' + (e.message || 'unknown error'));
      }
    },
    ...
    loadControlTrace: async function() {
      ...
      try {
        ...
        var session = await SiliCrewAPI.get('/api/agents/' + this.currentAgent.id + '/session');
        var sid = session && session.session_id;
        if (sid) {
          var data = await SiliCrewAPI.get('/api/sessions/' + sid + '/control-trace');
