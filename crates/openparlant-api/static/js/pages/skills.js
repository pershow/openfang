...
function skillsPage() {
  return {
    ...
    async loadSkills() {
      ...
      this.loadError = '';
      try {
        var data = await SiliCrewAPI.get('/api/skills');
    ...
    async searchClawHub() {
      ...
      this.clawhubError = '';
      try {
        var data = await SiliCrewAPI.get('/api/clawhub/search?q=' + encodeURIComponent(this.clawhubSearch.trim()) + '&limit=20');
    ...
    async browseClawHub(sort) {
      ...
      try {
        var data = await SiliCrewAPI.get('/api/clawhub/browse?sort=' + this.clawhubSort + '&limit=20');
    ...
    async loadMoreClawHub() {
      ...
      this.clawhubLoading = true;
      try {
        var data = await SiliCrewAPI.get('/api/clawhub/browse?sort=' + this.clawhubSort + '&limit=20&cursor=' + encodeURIComponent(this.clawhubNextCursor));
    ...
    async showSkillDetail(slug) {
      ...
      try {
        var data = await SiliCrewAPI.get('/api/clawhub/skill/' + encodeURIComponent(slug));
        this.skillDetail = data;
      } catch(e) {
        SiliCrewToast.error('Failed to load skill details');
      }
    ...
    async viewSkillCode(slug) {
      ...
      try {
        var data = await SiliCrewAPI.get('/api/clawhub/skill/' + encodeURIComponent(slug) + '/code');
        this.skillCode = data.code || '';
        this.skillCodeFilename = data.filename || 'source';
        this.showSkillCode = true;
      } catch(e) {
        SiliCrewToast.error('Could not load skill source code');
      }
    ...
    async installFromClawHub(slug) {
      ...
      this.installResult = null;
      try {
        var data = await SiliCrewAPI.post('/api/clawhub/install', { slug: slug });
        this.installResult = data;
        if (data.warnings && data.warnings.length > 0) {
          SiliCrewToast.success('Skill "' + data.name + '" installed with ' + data.warnings.length + ' warning(s)');
        } else {
          SiliCrewToast.success('Skill "' + data.name + '" installed successfully');
        }
        ...
        if (msg.includes('already_installed')) {
          SiliCrewToast.error('Skill is already installed');
        } else if (msg.includes('SecurityBlocked')) {
          SiliCrewToast.error('Skill blocked by security scan');
        } else {
          SiliCrewToast.error('Install failed: ' + msg);
        }
      }
    ...
    uninstallSkill: function(name) {
      var self = this;
      SiliCrewToast.confirm('Uninstall Skill', 'Uninstall skill "' + name + '"? This cannot be undone.', async function() {
        try {
          await SiliCrewAPI.post('/api/skills/uninstall', { name: name });
          SiliCrewToast.success('Skill "' + name + '" uninstalled');
          await self.loadSkills();
        } catch(e) {
          SiliCrewToast.error('Failed to uninstall skill: ' + e.message);
        }
      });
    },
    ...
    async createDemoSkill(skill) {
      try {
        await SiliCrewAPI.post('/api/skills/create', {
          name: skill.name,
          description: skill.description,
          runtime: 'prompt_only',
          prompt_context: skill.prompt_context || skill.description
        });
        SiliCrewToast.success('Skill "' + skill.name + '" created');
        this.tab = 'installed';
        await this.loadSkills();
      } catch(e) {
        SiliCrewToast.error('Failed to create skill: ' + e.message);
      }
    },
    ...
    async loadMcpServers() {
      ...
      try {
        var data = await SiliCrewAPI.get('/api/mcp/servers');
