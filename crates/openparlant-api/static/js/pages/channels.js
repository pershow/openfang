...
function channelsPage() {
  return {
    ...
    async loadChannels() {
      ...
      this.loadError = '';
      try {
        var data = await SiliCrewAPI.get('/api/channels');
    ...
    async startQR() {
      ...
      try {
        var result = await SiliCrewAPI.post('/api/channels/whatsapp/qr/start', {});
        ...
        if (this.qr.connected) {
          SiliCrewToast.success('WhatsApp connected!');
    ...
    pollQR() {
      ...
      this.qrPollTimer = setInterval(async function() {
        try {
          var result = await SiliCrewAPI.get('/api/channels/whatsapp/qr/status?session_id=' + encodeURIComponent(self.qr.sessionId));
          if (result.connected) {
            clearInterval(self.qrPollTimer);
            self.qrPollTimer = null;
            self.qr.connected = true;
            self.qr.message = result.message || 'Connected!';
            SiliCrewToast.success('WhatsApp linked successfully!');
    ...
    async saveChannel() {
      ...
      try {
        await SiliCrewAPI.post('/api/channels/' + name + '/configure', {
          fields: this.formValues
        });
        this.setupStep = 2;
        // Auto-test after save
        try {
          var testResult = await SiliCrewAPI.post('/api/channels/' + name + '/test', {});
          if (testResult.status === 'ok') {
            this.testPassed = true;
            this.setupStep = 3;
            SiliCrewToast.success(this.setupModal.display_name + ' activated!');
          } else {
            SiliCrewToast.success(this.setupModal.display_name + ' saved. ' + (testResult.message || ''));
          }
        } catch(te) {
          SiliCrewToast.success(this.setupModal.display_name + ' saved. Test to verify connection.');
        }
        await this.refreshStatus();
      } catch(e) {
        SiliCrewToast.error('Failed: ' + (e.message || 'Unknown error'));
      }
    ...
    async removeChannel() {
      ...
      var self = this;
      SiliCrewToast.confirm('Remove Channel', 'Remove ' + displayName + ' configuration? This will deactivate the channel.', async function() {
        try {
          await SiliCrewAPI.delete('/api/channels/' + name + '/configure');
          SiliCrewToast.success(displayName + ' removed and deactivated.');
          await self.refreshStatus();
    ...
    async testChannel() {
      ...
      try {
        var result = await SiliCrewAPI.post('/api/channels/' + name + '/test', {});
        if (result.status === 'ok') {
          this.testPassed = true;
          this.setupStep = 3;
          SiliCrewToast.success(result.message);
        } else {
          SiliCrewToast.error(result.message);
        }
      } catch(e) {
        SiliCrewToast.error('Test failed: ' + (e.message || 'Unknown error'));
      }
    ...
    async copyConfig(ch) {
      ...
      try {
        ...
        SiliCrewToast.success('Copied to clipboard');
      } catch(e) {
        SiliCrewToast.error('Copy failed');
      }
    },
