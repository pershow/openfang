# 基于 SiliCrew / OpenParlant 的企业级多-Agent 底座设计 v3（增量演进版）

## 1. 文档定位

本文档用于替代 v2 设计稿，吸收以下两份材料后的统一结论：

- `docs/基于SiliCrew改造的企业级多Agent底座-可动工设计-v2.md`
- `docs/建议.md`

本版不再把目标表述为“直接把 Parlant 和 SiliCrew 硬合并”，也不再把项目定义为“完全新写一套 Rust 底座”。本版明确采用：
...
**在当前 SiliCrew/OpenParlant Rust 仓库上，增量加入 Parlant 风格的 conversational control layer，先做出企业可控的前台对话底座，再逐步扩展到完整企业 Agent OS。**
...
1. **MVP 明确采用 PostgreSQL-first（pgsql），但以“统一主存”替代“同时完成多租户强隔离”**
...
- 决定继续以 SQLite 作为生产主存，仅把 PostgreSQL 作为可选兼容项
- 决定首版必须 day-1 支持真正的多租户强隔离、schema-per-tenant 或完整 RLS 体系
...
- Rust workspace，crate 已以 `silicrew-*` 命名
...
2. **先抽象存储边界并完成 PostgreSQL 主路径，不保留长期 SQLite 主路径**
3. **允许短期导入/校验窗口，但不接受 runtime 和 control-plane 长期双存储分裂**
4. **先沿用现有 API 与 UI，逐步扩展，不先并行起新 apps/server/worker/cli**
5. **先实现“对话可控”，再实现“企业平台化”**
...
来源于当前 SiliCrew/OpenParlant 仓库，负责：
...
**实现说明（兼容模式）**：在 `tool_exposure_policies` 的 PostgreSQL（pgsql）实现中，若某个 **scope 下尚无任何 tool policy 记录**，则该 scope 被视为**未接入控制面工具治理**，工具门控对候选工具保持 **legacy「无匹配策略则放行」**，以免破坏未配置策略的现有部署。一旦为该 scope **创建了任意一条** tool policy，该 scope 即切换为 **控制面托管模式（deny-by-default）**：仅匹配策略的工具可见，其余拒绝。这与上文「理想产品语义」的差异是刻意的迁移兼容策略；若需从首条配置起就等价于「默认不可见」，应在创建 scope 后立即添加覆盖所用工具名的策略（或后续提供显式 scope 级开关）。
...
当前 workspace 已使用 `silicrew-*` crate 命名。

因此本版拍板：

- **crate 命名继续沿用 `silicrew-*`**
- 产品层可以继续使用 “SiliCrew 企业版 / 企业底座” 叙事
- 不再混用 `silicrew-*`、`silicrew-*`、`silicrewos-*` 三套 crate 名
...
**PostgreSQL-first（pgsql）**

原因：

- 企业级多会话、审批、trace / replay、发布治理和报表查询更适合 PostgreSQL 的并发、事务与索引能力
- 后续 scope -> tenant 演进、审计追踪、版本发布与回滚都需要统一主存，避免二次迁移
- 如果先做 SQLite-first，再把 runtime 和 control-plane 迁到 PostgreSQL，整体迁移成本会更高
- 当前仓库虽深度依赖 SQLite，但这说明需要设计迁移路径，不说明应该继续把 SQLite 作为长期生产主存

工程约束：

- 新部署默认只支持 PostgreSQL，配置入口以 `DATABASE_URL` 为主
- SQLite 只用于本地开发导入、历史数据迁移或回归比对，不再作为长期运行时主库
- 不接受 runtime legacy tables 在 SQLite、control-plane tables 在 PostgreSQL 的长期裂脑状态

### 11.2 控制面表设计（MVP，PostgreSQL）

PostgreSQL 表设计约定：

- 内部主键统一优先使用 `uuid`
- `agent_id`、`session_id`、`external_user_id` 等与现有 runtime 强耦合的业务标识，MVP 阶段先保留为 `text`
- `matcher_config`、`config_json`、`state_payload`、`decision_json` 等结构化字段统一使用 `jsonb`
- 时间字段统一使用 `timestamptz`
- 所有 scope 级表默认建立 `(scope_id, enabled)`、`(scope_id, updated_at desc)` 或等价业务索引；`jsonb` 检索场景按需补充 GIN 索引
...
### 11.3 SQLite -> PostgreSQL 迁移路径

迁移不再后置到专题阶段，而是作为 MVP 的一部分落地：

1. 在 `silicrew-memory`、`silicrew-control`、`silicrew-policy`、`silicrew-journey`、`silicrew-context` 前先收口 repository / store trait，避免 `rusqlite::Connection` 继续向上泄漏
2. 为现有 `sqlx` 基础设施打开 PostgreSQL feature，引入统一 `PgPool`，并以 `DATABASE_URL` 作为主配置入口
3. 先落 control-plane schema 与 turn trace，再迁 session、usage、knowledge、structured memory 等 runtime 核心表
4. 提供一次性 `sqlite -> pgsql` importer，用于历史数据迁移、本地开发数据导入和回归校验
5. API / runtime 切到 PostgreSQL 后，SQLite 仅保留短期导入/回滚窗口，窗口结束后移除主路径

迁移原则：

- 不做长期双写
- 不让控制面先跑 PostgreSQL、runtime 继续长期停留 SQLite
- 必须把迁移工具、回归对账和灰度切换一起设计，而不是只改 migration 脚本
...
- 锁定本版架构
- 确认 PostgreSQL-first（pgsql）
- 确认 crate 命名沿用 `silicrew-*`
...
- 新 crate 清单确认
- API 扩展清单确认
- `DATABASE_URL` / `PgPool` 作为统一存储入口的拍板
...
- 建立 `silicrew-context`
- 建立 `silicrew-control`
- 收口 repository / store trait
- 建立控制面 PostgreSQL schema 与 migration
- 打通 `DATABASE_URL` / `PgPool`
...
- 能保存 turn trace
- PostgreSQL 可作为控制面与 trace 的主存启动
...
- journey runtime
- retriever / glossary / variables
- 迁移 session / usage / knowledge 等核心运行时读写到 PostgreSQL
...
### 17.1 PostgreSQL 改造低估风险

如果团队把“改成 pgsql”理解成只需要替换 migration 或连接串，而忽略 `rusqlite::Connection` 渗透、事务边界、导入工具和回归对账，项目会失控。

处理原则：

- 先收口存储接口，再替换底层实现
- 允许短期导入窗口，不接受长期双写
- 把运行时主链路切换和控制面上线放在同一实施计划内
...
2. **MVP 采用 PostgreSQL-first（pgsql），SQLite 仅保留导入与回归用途**
3. **crate 命名保持 `silicrew-*`**
...
1. 新增 `silicrew-policy`
2. 新增 `silicrew-journey`
3. 新增 `silicrew-context`
4. 新增 `silicrew-control`
5. 抽离 repository / store trait，收口 `rusqlite::Connection`
6. 生成 PostgreSQL migration 与 `sqlite -> pgsql` importer
7. 定义 `CompiledTurnContext` 与 `TurnControlCoordinator`
8. 在现有 WebChat 中先接一版只读 trace 面板

如果按这个顺序推进，项目会先形成“控制面接上现有执行面 + PostgreSQL 主存落地”的最小闭环，而不是先陷入长期双存储的基础设施泥潭。
